...
#define iOS8before [[UIDevice currentDevice].systemVersion floatValue] < 8.0
#define kViewBgColor [UIColor colorWithRed:244/255.0 green:244/255.0 blue:244/255.0 alpha:1]
#define kBtnBgColor [UIColor colorWithRed:253/255.0 green:78/255.0 blue:84/255.0 alpha:1]
...
- (void)creatShadowView {
    _shadowView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, _contentViewWidth, 225)];
...
//            [btn setTitle:confirmTitle forState:UIControlStateNormal];
            btn.layer.masksToBounds = YES;
            btn.layer.cornerRadius = kBtnCorner;
        }
    }
...
- (void)updateTitleLabelFrame {
    ...
    if (self.title.length) {
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(8, 18, 20, 20)];
        imageView.image = [UIImage imageNamed:@"u261"];
        [_contentView addSubview:imageView];
        CGSize size = [self.titleLabel sizeThatFits:CGSizeMake(labelWidth, CGFLOAT_MAX)];
        titleHeight = size.height;
        self.titleLabel.frame = CGRectMake(35, 18, labelWidth, size.height);
    }
}
...
- (void)updateAllButtonsFrame {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        btn.frame = CGRectMake(buttonX, buttonY + 20, buttonWidth - 30, _buttonHeight - 10);
    }
}
...
- (void)updateShadowAndContentViewFrame {
    ...
    frame.size.height = firstButtonY+allButtonHeight + 30;
    _shadowView.frame = frame;
    
    _shadowView.center = self.view.center;
    _contentView.frame = _shadowView.bounds;
    
    UIView *oneView = [[UIView alloc]initWithFrame:CGRectMake(0, _titleLabel.frame.origin.y+_titleLabel.frame.size.height + 8, _contentView.frame.size.width, 0.5)];
    oneView.backgroundColor = [UIColor colorWithWhite:0.94 alpha:1];
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [self creatLabelWithFontSize:17];
